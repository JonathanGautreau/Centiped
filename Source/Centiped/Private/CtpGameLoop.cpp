...
void UCtpGameLoop::SpawnMushrooms(UWorld* World, ACtpGameMode* GameMode, int MushroomsCount, int RowMin, int RowMax)
{
	...
	while (SpawnedMushrooms < MushroomsCount && AvailableCells.Num() > 0)
	{
		...
		if (!(Col > PlayerSquare.X && Col < PlayerSquare.Y && Row > PlayerSquare.Z) && NumberOfDeletedCells > 0)
		{
			...
			int x = round(GameMode->Bounds.Min.X) + round(Mushroom->MeshScale.X * 100 * Col) + round(
				Mushroom->MeshScale.X * 0.5 * 100);
			int y = round(GameMode->Bounds.Max.Y) - round(Mushroom->MeshScale.Y * 100 * Row) - round(
				Mushroom->MeshScale.Y * 0.5 * 100);
...
int UCtpGameLoop::CountMushroomInPlayerZone()
{
	...
	for (TActorIterator<AActor> It(GetWorld()); It; ++It)
	{
		if (ACtpMushroom* Mushroom = Cast<ACtpMushroom>(*It))
		{
			...
			{
				double PlayerVerticalLimit = GameMode->Bounds.Max.Y - round(
					GameMode->SquareSize.Y * FMath::RoundToInt(GameMode->Rows * 0.7f));
...
void UCtpGameLoop::GenerateCentipede(UWorld* World, FActorSpawnParameters& SpawnParams, ACtpGameMode* GameMode)
{
	...
	for (int i = 0; i < CentipedeSize; ++i)
	{
		...
		if (Prev)
		{
			Curr->HitSwitches.EmplaceAt(0, FVector(0.f, GameMode->Bounds.Min.X + Curr->MeshScale.Y * 100 * 0.5,
			                                       GameMode->Bounds.Max.Y - Curr->MeshScale.Y * 100 * 0.5));
			Prev->NextNode = Curr;
			Curr->SetActorLocation(FVector(0, Prev->GetActorLocation().Y + Curr->MeshScale.X * 100,
			                               Prev->GetActorLocation().Z));
		}
		else
		{
...
void UCtpGameLoop::GenerateFlea()
{
	if (UWorld* World = GetWorld())
	{
		if (ACtpGameMode* GameMode = Cast<ACtpGameMode>(World->GetAuthGameMode()))
		{
			...
			float SpawnOnY = FMath::RandRange(GameMode->Bounds.Min.X + Flea->MeshScale.X * 100,
			                                  GameMode->Bounds.Max.X - Flea->MeshScale.X * 100);
			Flea->SetActorLocation(FVector(0, SpawnOnY, GameMode->Bounds.Max.Y) - Flea->MeshScale.Y * 100);
			Flea->HitSwitch = FVector2D(
				SpawnOnY, GameMode->Bounds.Max.Y - Flea->MeshScale.Y * 200 - Flea->VerticalOffset);
		}
	}
}
	...
	if (UWorld* World = GetWorld())
	{
		if (ACtpGameMode* GameMode = Cast<ACtpGameMode>(World->GetAuthGameMode()))
		{
			...
			float SpawnOnZ = FMath::RandRange(
				GameMode->Bounds.Max.Y - round(GameMode->SquareSize.Y * FMath::RoundToInt(GameMode->Rows * 0.7f)) -
				round(GameMode->SquareSize.Y * 0.5), GameMode->Bounds.Max.Y);
	...
	if (UWorld* World = GetWorld())
	{
		if (ACtpGameMode* GameMode = Cast<ACtpGameMode>(World->GetAuthGameMode()))
		{
			...
			float SpawnOnZ = FMath::RandRange(
				...
				GameMode->Bounds.Max.Y - round(GameMode->SquareSize.Y * FMath::RoundToInt(GameMode->Rows * 0.7f)) -
				round(GameMode->SquareSize.Y * 0.5));
			int randomYPosition = FMath::RandBool() ? 1 : -1;
			// Spawn on right, going left
			if (FMath::RandBool())
			{
				Spider->Direction = FVector2D(-1, randomYPosition);
				Spider->SetActorLocation(FVector(0, GameMode->Bounds.Max.X - Spider->MeshScale.X * 100 * 0.5,
				                                 SpawnOnZ));
			}
			...
			{
				Spider->Direction = FVector2D(1, randomYPosition);
				Spider->SetActorLocation(FVector(0, GameMode->Bounds.Min.X + Spider->MeshScale.X * 100 * 0.5,
				                                 SpawnOnZ));
			}
		}
	}
